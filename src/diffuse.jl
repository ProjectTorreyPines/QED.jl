...
function define_T(QI::QED_state)
    ...
    Threads.@threads for m in 1:N
        ...
        Ts[2m-1, 1] = 0.0
        if m > 1
            Ts[2m-1, 2] = inner_product(DΦ, νo, m, νo, m-1, ρ, order=5)
            Ts[2m-1, 3] = inner_product(DΦ, νo, m, νe, m-1, ρ, order=5)
        end
        Ts[2m-1, 4] = inner_product(DΦ, νo, m, νo, m, ρ, order=5)
        Ts[2m-1, 5] = inner_product(DΦ, νo, m, νe, m, ρ, order=5)
        if m < N
            Ts[2m-1, 6] = inner_product(DΦ, νo, m, νo, m+1, ρ, order=5)
            Ts[2m-1, 7] = inner_product(DΦ, νo, m, νe, m+1, ρ, order=5)
        end

        # Then even with all nearest neighbors
        if m > 1
            Ts[2m, 1] = inner_product(DΦ, νe, m, νo, m-1, ρ, order=5)
            Ts[2m, 2] = inner_product(DΦ, νe, m, νe, m-1, ρ, order=5)
        end
        Ts[2m, 3] = inner_product(DΦ, νe, m, νo, m, ρ, order=5)
        Ts[2m, 4] = inner_product(DΦ, νe, m, νe, m, ρ, order=5)
        if m < N
            Ts[2m, 5] = inner_product(DΦ, νe, m, νo, m+1, ρ, order=5)
            Ts[2m, 6] = inner_product(DΦ, νe, m, νe, m+1, ρ, order=5)
        end
        Ts[2m, 7] = 0.0
    end
